...
contract('BActions', async (accounts) => {
    ...
    describe('Private pool controls', () => {
        ...
        before(async () => {
            ...
            const params = [FACTORY, createTokens, createBalances, createWeights, swapFee, protocolFee, finalize];

            const functionCall = web3.eth.abi.encodeFunctionCall(createInterface, params);

            const poolAddress = await userProxy.methods['execute(address,bytes)'].call(BACTIONS, functionCall);
        ...
        it('rebind add new token', async () => {
            ...
            const params = [POOL, rebindTokens, rebindBalances, rebindWeights];
        ...
        it('fails other user interaction', async () => {
            ...
            await truffleAssert.reverts(bpool.rebind(ZRX, toWei('5'), toWei('5')), 'ERR_NOT_CONTROLLER');
