...
contract('BActions', async (accounts) => {
    ...
    describe('Smart pool', () => {
        ...
        it('allows joining from user', async () => {
            ...
            const joinInterface = BActions.abi.find((iface) => iface.name === 'joinSmartPool');
            const params = [POOL, toWei('80'), [toWei('200'), toWei('0.5'), toWei('2')]];
        ...
        it('allows joinswap from user', async () => {
            ...
            const joinswapExternAmountInInterface = BActions.abi.find(
                (iface) => iface.name === 'joinswapExternAmountIn'
            );
            const params = [POOL, MKR, toWei('0.3'), toWei('14')];
            ...
            const functionCall = web3.eth.abi.encodeFunctionCall(joinswapExternAmountInInterface, params);
            await userProxy.methods['execute(address,bytes)'](BACTIONS, functionCall, {
                from: user,
            });
