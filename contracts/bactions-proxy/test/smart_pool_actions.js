...
const BPool = artifacts.require('contracts/test/BPool.sol:BPool');
const ConfigurableRightsPool = artifacts.require('contracts/test/ConfigurableRightsPool.sol:ConfigurableRightsPool');
...
contract('BActions', async (accounts) => {
    ...
    describe('Smart pool actions', () => {
        ...
        beforeEach(async () => {
            ...
            const params = [CRP_FACTORY, FACTORY, poolParams, crpParams, rights];

            const functionCall = web3.eth.abi.encodeFunctionCall(createSmartPoolInterface, params);

            const poolAddress = await creatorProxy.methods['execute(address,bytes)'].call(BACTIONS, functionCall);
        ...
        it('does not allow joining', async () => {
            ...
            const params = [POOL, toWei('40'), [toWei('80'), toWei('0.2'), toWei('0.8')]];
        ...
        it('does not allow joinswap', async () => {
            const joinswapInterface = BActions.abi.find((iface) => iface.name === 'joinswapExternAmountIn');
            const params = [POOL, MKR, toWei('0.3'), toWei('7')];
        ...
        it('does not allow changing params by user', async () => {
            const setPublicSwapInterface = BActions.abi.find((iface) => iface.name === 'setPublicSwap');
            let params = [POOL, false];

            let functionCall = web3.eth.abi.encodeFunctionCall(setPublicSwapInterface, params);
            await truffleAssert.fails(
            ...
            params = [POOL, toWei('0.01')];

            functionCall = web3.eth.abi.encodeFunctionCall(setSwapFeeInterface, params);
            await truffleAssert.fails(
                userProxy.methods['execute(address,bytes)'](BACTIONS, functionCall, { from: user }),
                truffleAssert.ErrorType.REVERT,
                // 'ERR_NOT_CONTROLLER'
            ...
            const setControllerInterface = BActions.abi.find((iface) => iface.name === 'setController');
            params = [POOL, USER_PROXY];

            functionCall = web3.eth.abi.encodeFunctionCall(setControllerInterface, params);
            await truffleAssert.fails(
                userProxy.methods['execute(address,bytes)'](BACTIONS, functionCall, { from: user }),
                truffleAssert.ErrorType.REVERT,
                // 'ERR_NOT_CONTROLLER'
        ...
        it('allows toggling public swap', async () => {
            ...
            const setPublicSwapInterface = BActions.abi.find((iface) => iface.name === 'setPublicSwap');
            const params = [POOL, false];

            const functionCall = web3.eth.abi.encodeFunctionCall(setPublicSwapInterface, params);
            await creatorProxy.methods['execute(address,bytes)'](BACTIONS, functionCall);
        ...
        it('allows changing owner', async () => {
            ...
            const setControllerInterface = BActions.abi.find((iface) => iface.name === 'setController');
            const params = [POOL, USER_PROXY];

            const functionCall = web3.eth.abi.encodeFunctionCall(setControllerInterface, params);
            await creatorProxy.methods['execute(address,bytes)'](BACTIONS, functionCall);
        ...
        it('allows increasing token weight', async () => {
            ...
            const setCapInterface = BActions.abi.find((iface) => iface.name === 'setCap');
            let params = [POOL, toWei('300')];

            let functionCall = web3.eth.abi.encodeFunctionCall(setCapInterface, params);
            await creatorProxy.methods['execute(address,bytes)'](BACTIONS, functionCall);

            const increaseWeightInterface = BActions.abi.find((iface) => iface.name === 'increaseWeight');
            params = [POOL, MKR, toWei('20'), toWei('1')];
            ...
            functionCall = web3.eth.abi.encodeFunctionCall(increaseWeightInterface, params);
            await creatorProxy.methods['execute(address,bytes)'](BACTIONS, functionCall);
        ...
        it('allows decreasing token weight', async () => {
            ...
            const decreaseWeightInterface = BActions.abi.find((iface) => iface.name === 'decreaseWeight');
            const params = [POOL, DAI, toWei('2'), toWei('40')];
            ...
            const functionCall = web3.eth.abi.encodeFunctionCall(decreaseWeightInterface, params);
            await creatorProxy.methods['execute(address,bytes)'](BACTIONS, functionCall);
        ...
        it('allows updating token weights gradually', async () => {
            ...
            const updateWeightsGraduallyInterface = BActions.abi.find(
                (iface) => iface.name === 'updateWeightsGradually'
            );
            const params = [POOL, newWeights, startBlock, endBlock];
            const functionCall = web3.eth.abi.encodeFunctionCall(updateWeightsGraduallyInterface, params);
            await creatorProxy.methods['execute(address,bytes)'](BACTIONS, functionCall);
        ...
        it('allows changing cap', async () => {
            ...
            const setCapInterface = BActions.abi.find((iface) => iface.name === 'setCap');
            const params = [POOL, toWei('500')];

            const functionCall = web3.eth.abi.encodeFunctionCall(setCapInterface, params);
            await creatorProxy.methods['execute(address,bytes)'](BACTIONS, functionCall);
        ...
        it('allows adding token to the pool', async () => {
            ...
            // Increase cap
            const setCapInterface = BActions.abi.find((iface) => iface.name === 'setCap');
            let params = [POOL, toWei('500')];
            let functionCall = web3.eth.abi.encodeFunctionCall(setCapInterface, params);
            await creatorProxy.methods['execute(address,bytes)'](BACTIONS, functionCall);

            // Commit
            const commitAddTokenInterface = BActions.abi.find((iface) => iface.name === 'commitAddToken');
            params = [POOL, BAL, toWei('5'), toWei('10')];
            functionCall = web3.eth.abi.encodeFunctionCall(commitAddTokenInterface, params);
            await creatorProxy.methods['execute(address,bytes)'](BACTIONS, functionCall);
            ...
            // Apply
            const applyAddTokenInterface = BActions.abi.find((iface) => iface.name === 'applyAddToken');
            params = [POOL, BAL, toWei('5')];
            functionCall = web3.eth.abi.encodeFunctionCall(applyAddTokenInterface, params);
            await creatorProxy.methods['execute(address,bytes)'](BACTIONS, functionCall);
        ...
        it('allows removing token from the pool', async () => {
            ...
            const removeTokenInterface = BActions.abi.find((iface) => iface.name === 'removeToken');
            const params = [POOL, DAI, toWei('50')];

            await crp.approve(CREATOR_PROXY, MAX);

            const functionCall = web3.eth.abi.encodeFunctionCall(removeTokenInterface, params);
            await creatorProxy.methods['execute(address,bytes)'](BACTIONS, functionCall);
        ...
        it('allows adding provider to the list', async () => {
            ...
            const whitelistLPInterface = BActions.abi.find((iface) => iface.name === 'whitelistLiquidityProvider');
            let params = [POOL, [CREATOR_PROXY]];
            let functionCall = web3.eth.abi.encodeFunctionCall(whitelistLPInterface, params);
            await creatorProxy.methods['execute(address,bytes)'](BACTIONS, functionCall);

            params = [POOL, [USER_PROXY]];
            functionCall = web3.eth.abi.encodeFunctionCall(whitelistLPInterface, params);
            await creatorProxy.methods['execute(address,bytes)'](BACTIONS, functionCall);
        ...
        it('allows removing provider from the list', async () => {
            ...
            const whitelistLPInterface = BActions.abi.find((iface) => iface.name === 'whitelistLiquidityProvider');
            const params = [POOL, [CREATOR_PROXY]];
            let functionCall = web3.eth.abi.encodeFunctionCall(whitelistLPInterface, params);
            await creatorProxy.methods['execute(address,bytes)'](BACTIONS, functionCall);
            ...
            const removeWhitelistedLPInterface = BActions.abi.find(
                (iface) => iface.name === 'removeWhitelistedLiquidityProvider'
            );
            functionCall = web3.eth.abi.encodeFunctionCall(removeWhitelistedLPInterface, params);
            await creatorProxy.methods['execute(address,bytes)'](BACTIONS, functionCall);
