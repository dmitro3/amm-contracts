...
contract('Bankless Simulation', async (accounts) => {
    ...
    it('Should not allow anyone to add liquidity', async () => {
        await truffleAssert.reverts(crpPool.joinswapPoolAmountOut.call(DAI, toWei('1'), MAX), 'ERR_CAP_LIMIT_REACHED');
    ...
    describe('BAP0 shirt auction', () => {
        ...
        it('Controller should recover remaining tokens and proceeds', async () => {
            ...
            while (Decimal(fromWei(poolDaiBalance)) > 0.001 && Decimal(fromWei(poolShirtBalance)) > 0.001) {
                ...
                await crpPool.exitswapExternAmountOut(DAI, toWei(daiWithdrawal.toString()), toWei(numPoolTokens));
                await crpPool.exitswapExternAmountOut(BAP0, toWei(shirtWithdrawal.toString()), toWei(numPoolTokens));
