...
const { calcInGivenOut, calcOutGivenIn, calcRelativeDiff } = require('../calc_comparisons');
...
contract('Bankless Simulation (destroy pool)', async (accounts) => {
    ...
    before(async () => {
        ...
        restrictedRole = await fcxAclInstance.RESTRICTED_ROLE();
        unRestrictedRole = await fcxAclInstance.UNRESTRICTED_ROLE();
        ...
        CRPPOOL = await crpFactory.newCrp.call(bFactory.address, poolParams, permissions, { from: admin });
        ...
        await fcxAclInstance.grantRoles(
            [
                ...
                { account: user2, role: unRestrictedRole },
                { account: user3, role: unRestrictedRole },
            ],
            { from: admin }
        );
    });
    ...
    it('Should not allow anyone to add liquidity', async () => {
        await truffleAssert.reverts(crpPool.joinswapPoolAmountOut.call(DAI, toWei('1'), MAX), 'ERR_NOT_ON_WHITELIST');
    ...
    describe('BAP0 shirt auction', () => {
        it('Should configure the pool (min swap fee)', async () => {
            ...
            const underlyingPool = await IBPool.at(bPoolAddr);
        ...
        it('Should not allow removing a token during an update', async () => {
            await truffleAssert.reverts(crpPool.removeToken(DAI), 'ERR_NO_UPDATE_DURING_GRADUAL');
        });
        ...
        it('Should be able to poke weights (and buy shirts)', async () => {
            ...
            while (shirtsLeft) {
                ...
                console.log(
                    `User ${userIdx + 1} has ${Math.round(fromWei(daiBalance))} Dai and ${fromWei(bap0Balance)} shirts.`
                );
                console.log(
                ...
                console.log(
                    `Actual cost: ${Decimal(fromWei(tokenAmountIn)).toFixed(2)}; spot price after is ${Decimal(
                        fromWei(spotPriceAfter)
                    ).toFixed(2)}`
                );
