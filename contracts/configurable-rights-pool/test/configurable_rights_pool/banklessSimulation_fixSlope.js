...
contract('Bankless Simulation (mid-stream adjustment)', async (accounts) => {
    ...
    before(async () => {
        ...
        await fcxAclInstance.grantRoles([{ account: user, role: unRestrictedRole }], { from: admin });
    });
            ...
            for (i = 0; i < 5; i++) {
                ...
                console.log(
                    '\nBlock: ' +
                        block.number +
                        '. Weights -> BAP0: ' +
                        ((weightBap0 * 2.5) / 10 ** 18).toFixed(4) +
                        '%\tDAI: ' +
                        ((weightDai * 2.5) / 10 ** 18).toFixed(2) +
                        '%'
                );
                console.log(
                    `Raw weights: ${Decimal(fromWei(weightBap0)).toFixed(4)} / ${Decimal(fromWei(weightDai)).toFixed(
                        2
                    )}`
                );
                ...
                console.log(
                    `User has ${Decimal(fromWei(daiBalance)).toFixed(2)} Dai and ${Decimal(
                        fromWei(bap0Balance)
                    ).toFixed(1)} shirts.`
                );
                console.log(
                    `Pool has ${Decimal(fromWei(tokenOutBalance)).toFixed(1)} shirts / ${Decimal(
