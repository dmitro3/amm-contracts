...
contract('BSP Cap', async (accounts) => {
    ...
    it('Should not allow setting the cap with no pool yet', async () => {
        await truffleAssert.reverts(crpPool.setCap(toWei('1000')), 'ERR_NOT_CREATED');
    });
    ...
    it('Set cap should revert for non-controller', async () => {
        await truffleAssert.reverts(crpPool.setCap(toWei('1000'), { from: user1 }), 'ERR_NOT_CONTROLLER');
    });
    ...
    describe('Joining pools', () => {
        ...
        it('Should allow LPs to join', async () => {
            ...
            while (expectedSupply < 200) {
                ...
                const daiCost = await crpPool.joinswapPoolAmountOut.call(DAI, toWei(amountOut.toString()), MAX, {
                    from: user,
                });
                console.log(
