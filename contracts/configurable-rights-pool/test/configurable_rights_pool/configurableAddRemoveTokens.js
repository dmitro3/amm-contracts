...
contract('configurableAddRemoveTokens', async (accounts) => {
    ...
    before(async () => {
        ...
        */
        ...
        xyz = await ERC20Mock.new('XYZ', 'XYZ', 18);
    ...
    it('Controller should not be able to commitAddToken with invalid weight', async () => {
        await truffleAssert.reverts(crpPool.commitAddToken(ABC, toWei('10000'), toWei('50.1')), 'ERR_WEIGHT_ABOVE_MAX');

        await truffleAssert.reverts(crpPool.commitAddToken(ABC, toWei('10000'), toWei('0.1')), 'ERR_WEIGHT_BELOW_MIN');
    ...
    it('Controller should not be able to commitAddToken with invalid balance', async () => {
        await truffleAssert.reverts(crpPool.commitAddToken(ABC, toWei('0'), toWei('5')), 'ERR_BALANCE_BELOW_MIN');
    });
    ...
    it('Controller should not be able to applyAddToken for a token that is already bound', async () => {
        truffleAssert.reverts(crpPool.commitAddToken(WETH, toWei('20'), toWei('1.5')), 'ERR_IS_BOUND');
    });

    it('Controller should not be able to applyAddToken for no commitment', async () => {
        truffleAssert.reverts(crpPool.applyAddToken(), 'ERR_NO_TOKEN_COMMIT');
    });
    ...
    it('Controller should be able to commitAddToken', async () => {
        ...
        await truffleAssert.reverts(bPool.getDenormalizedWeight.call(abc.address), 'ERR_NOT_BOUND');
    ...
    it('Should not be able to removeToken while an add is pending', async () => {
        await truffleAssert.reverts(crpPool.removeToken.call(xyz.address), 'ERR_REMOVE_WITH_ADD_PENDING');
    });
    ...
    it('Controller should not be able to applyAddToken before addTokenTimeLockInBlocks', async () => {
        ...
        while (block.number < applyAddTokenValidBlock) {
            ...
            await truffleAssert.reverts(crpPool.applyAddToken(), 'ERR_TIMELOCK_STILL_COUNTING');
    ...
    it('Non Controller account should not be able to applyAddToken', async () => {
        await truffleAssert.reverts(crpPool.applyAddToken({ from: accounts[1] }), 'ERR_NOT_CONTROLLER');
    });
    ...
    it('Controller should not be able to removeToken if token is not bound', async () => {
        truffleAssert.reverts(crpPool.removeToken(ASD), 'ERR_NOT_BOUND');
    });

    it('Non Controller account should not be able to removeToken if token is bound', async () => {
        await truffleAssert.reverts(crpPool.removeToken(DAI, { from: accounts[1] }), 'ERR_NOT_CONTROLLER');
    });
    ...
    it('Controller should be able to removeToken if token is bound', async () => {
        ...
        await truffleAssert.reverts(bPool.getDenormalizedWeight.call(dai.address), 'ERR_NOT_BOUND');
    ...
    it('Set swap fee should revert because non-permissioned', async () => {
        await truffleAssert.reverts(crpPool.setSwapFee(toWei('0.01')), 'ERR_NOT_CONFIGURABLE_SWAP_FEE');
    });
    ...
    it('Should fail when adding a token without enough token balance', async () => {
        ...
        await truffleAssert.reverts(crpPool.applyAddToken(), 'ERR_INSUFFICIENT_BAL');
    });

    it('Should readd token after remove', async () => {
        await crpPool.commitAddToken(XYZ, toWei('100000'), toWei('1.5'));
