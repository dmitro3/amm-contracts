...
contract('configurableLPNoWhitelist', async (accounts) => {
    ...
    before(async () => {
        ...
        fcxAclInstance.grantRole(adminRole, crpPool.address, { from: admin });
    ...
    it('Cannot whitelist without permission', async () => {
        await truffleAssert.reverts(crpPool.whitelistLiquidityProvider([admin]), 'ERR_CANNOT_WHITELIST_LPS');
    });
...
contract('configurableLP', async (accounts) => {
    ...
    it('With whitelisting off, anyone can be an LP initially', async () => {
        await crpPool.setCanWhitelistLPs(false);

        let hasPerm = await crpPool.canProvideLiquidity(admin);
        assert.isTrue(hasPerm, 'Admin can provide liquidity');
        ...
        await crpPool.setCanWhitelistLPs(true);
    });
    ...
    describe('Whitelist admin and admin3', async () => {
        ...
        it('Cannot whitelist null', async () => {
            await truffleAssert.reverts(crpPool.whitelistLiquidityProvider([ZERO_ADDRESS]), 'ERR_INVALID_ADDRESS');
        });
        ...
        it('Cannot remove if not on whitelist', async () => {
            await truffleAssert.reverts(crpPool.removeWhitelistedLiquidityProvider([admin2]), 'ERR_LP_NOT_WHITELISTED');
        });
        ...
        it('admin2 cannot join pool', async () => {
            ...
            await truffleAssert.reverts(
                crpPool.joinswapExternAmountIn(weth.address, toWei('100'), toWei('1'), {
                    from: admin2,
                }),
                'ERR_NOT_ON_WHITELIST'
            );
            ...
            await truffleAssert.reverts(
                crpPool.joinswapPoolAmountOut(weth.address, toWei('1'), toWei('100'), {
                    from: admin2,
                }),
                'ERR_NOT_ON_WHITELIST'
            );
