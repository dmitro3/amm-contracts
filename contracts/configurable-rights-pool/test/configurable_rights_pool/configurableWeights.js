...
contract('configurableWeights', async (accounts) => {
    ...
    describe('Weights permissions, etc', () => {
        ...
        it('Should not change weights below min', async () => {
            await truffleAssert.reverts(crpPool.updateWeight(WETH, toWei('0.1')), 'ERR_MIN_WEIGHT');
        });

        it('Should not change weights above max', async () => {
            await truffleAssert.reverts(crpPool.updateWeight(WETH, toWei('50.1')), 'ERR_MAX_WEIGHT');
        });

        it('Should not change weights if brings total weight above max', async () => {
            await truffleAssert.reverts(crpPool.updateWeight(WETH, toWei('37.5')), 'ERR_MAX_TOTAL_WEIGHT');
        });
        ...
        it('Controller should not be able to change weights when they do not have enough tokens', async () => {
            ...
            await truffleAssert.reverts(crpPool.updateWeight(WETH, toWei('4.5')), 'ERR_INSUFFICIENT_BAL');
        });

        it('Should not be able to update weight for non-token', async () => {
            // This should triple WETH weight from 1.5 to 4.5, requiring 80 WETH, but admin only has 60.
            await truffleAssert.reverts(crpPool.updateWeight(ABC, toWei('4.5')), 'ERR_NOT_BOUND');
        });
    });
    ...
    describe('updateWeightsGradually', () => {
        ...
        it('Controller should be able to call updateWeightsGradually() with valid range', async () => {
            ...
            console.log(
                `Gradual Update: ${startingXyzWeight}->${endXyzWeight}, ${startingWethWeight}->${endWethWeight}, ${startingDaiWeight}->${endDaiWeight}`
            );
        ...
        it('Should not be able to pokeWeights until valid start block reached', async () => {
            ...
            while (block.number < validStartBlock - 1) {
                await truffleAssert.reverts(crpPool.pokeWeights(), 'ERR_CANT_POKE_YET');
        ...
        it('Should run full update cycle, no missed blocks. (anyone can pokeWeights())', async () => {
            ...
            while (block.number < validEndBlock) {
                ...
                const newWethW = newWeight(
                    ...
                    validStartBlock,
                    validEndBlock,
                    Number(updatedWethWeight),
                    Number(endWethWeight)
                );
                const newDaiW = newWeight(
                    block,
                    validStartBlock,
                    validEndBlock,
                    Number(startingDaiWeight),
                    Number(endDaiWeight)
                );
                console.log(
                    `${block.number} Weights: ${newXyzW}/${fromWei(xyzWeight)}, ${newWethW}/${fromWei(
                        wethWeight
                    )}, ${newDaiW}/${fromWei(daiWeight)}`
                );
        ...
        it('Confirm update weights can be run with expired start block', async () => {
            ...
            console.log('Poking...');
            console.log(
                `${block.number} Weights: ${Decimal(fromWei(xyzWeight)).toFixed(4)} ${Decimal(
                    fromWei(wethWeight)
                ).toFixed(4)} ${Decimal(fromWei(daiWeight)).toFixed(4)}`
            );
        ...
        it('Remove token should revert because non-permissioned', async () => {
            await truffleAssert.reverts(crpPool.removeToken(DAI), 'ERR_CANNOT_ADD_REMOVE_TOKENS');
        });
        ...
        it('Apply add token should revert because non-permissioned', async () => {
            await truffleAssert.reverts(crpPool.applyAddToken(), 'ERR_CANNOT_ADD_REMOVE_TOKENS');
        });
    });
});
