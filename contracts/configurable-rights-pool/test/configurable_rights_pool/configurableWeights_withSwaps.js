...
contract('configurableWeights_withSwaps', async (accounts) => {
    ...
    describe('CWS Factory', () => {
        ...
        before(async () => {
            ...
            CONTROLLER = await factory.newCrp.call(bfactory.address, poolParams, permissions);

            await factory.newCrp(bfactory.address, poolParams, permissions);

            controller = await IConfigurableRightsPool.at(CONTROLLER);
        ...
        it('Should revert because too early to pokeWeights()', async () => {
            ...
            truffleAssert.reverts(controller.pokeWeights(), 'ERR_CANT_POKE_YET');
        });
        ...
        it('Should be able to pokeWeights()', async () => {
            ...
            truffleAssert.reverts(underlyingPool.finalize(), 'ERR_NOT_CONTROLLER');
