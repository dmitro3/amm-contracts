...
contract('crpPoolOverloadTests', async (accounts) => {
    ...
    const { toWei, fromWei, toTwosComplement } = web3.utils;
    ...
    before(async () => {
        ...
        crpPoolExtension = await IConfigurableRightsPool.at(CRPPOOL);
    ...
    it('crpPool should not create pool with inconsistent time parameters', async () => {
        await truffleAssert.reverts(crpPool.createPool(toWei('100'), 10, 20), 'ERR_INCONSISTENT_TOKEN_TIME_LOCK');
    });

    it('crpPool should not create pool with negative time parameters', async () => {
        await truffleAssert.reverts(
            // 0 > -10, but still shouldn't work
            crpPool.createPool(toWei('100'), 0, toTwosComplement(toWei(String(-10)))),
