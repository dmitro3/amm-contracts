...
const { calcOutGivenIn, calcRelativeDiff } = require('../calc_comparisons');
...
contract('crpPoolSwapIns', async (accounts) => {
    ...
    before(async () => {
        ...
        await weth.mint(nonWhitelistUser, toWei('25'));
        await dai.mint(nonWhitelistUser, toWei('30'));
        await xyz.mint(nonWhitelistUser, toWei('20'));
    ...
    it('Admin claim an amount from protocolFee', async () => {
        ...
        const expectedWethClaimed = await bMath.bmul(protocolFee, toWei('0.5'));
        const newAdminWethBalance = await weth.balanceOf(admin);
        const wethDiff = await bMath.bsub(newAdminWethBalance, adminWethBalance);
        assert.equal(fromWei(wethDiff), fromWei(expectedWethClaimed));
        ...
        const expectedDaiClaimed = await bMath.bmul(protocolFee, toWei('500'));
        const newAdminDaiBalance = await dai.balanceOf(admin);
        const daiDiff = await bMath.bsub(newAdminDaiBalance, adminDaiBalance);
        assert.equal(fromWei(daiDiff), fromWei(expectedDaiClaimed));

        // 3rd Swap XYZ for WETH
        await bPool3.swapExactAmountIn(
            XYZ,
            toWei('10'), // tokenAmountIn
            WETH,
            toWei('0'), // minAmountOut
        ...
        const expectedXYZClaimed = await bMath.bmul(protocolFee, toWei('10'));
        const newAdminXYZBalance = await xyz.balanceOf(admin);
        const xyzDiff = await bMath.bsub(newAdminXYZBalance, adminXYZBalance);
        assert.equal(fromWei(xyzDiff), fromWei(expectedXYZClaimed));
    });
});
