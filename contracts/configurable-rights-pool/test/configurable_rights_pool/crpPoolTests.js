...
const { calcRelativeDiff } = require('../calc_comparisons');
...
contract('crpPoolTests', async (accounts) => {
    ...
    const nonWhitelistUser = accounts[3];
    ...
    if (
        ('crpPool should not allow changing controller from non-admin',
        async () => {
            await truffleAssert.reverts(crpPool.setController(user1, { from: user1 }), 'ERR_NOT_CONTROLLER');
        })
    );

    if (
        ('crpPool should not allow changing controller to zero',
        async () => {
            await truffleAssert.reverts(crpPool.setController(ZERO_ADDRESS), 'ERR_ZERO_ADDRESS');
        })
    );

    if (
        ('crpPool should allow changing controller by admin',
        async () => {
    ...
    it('crpPool should not createPool with 0 BPT Initial Supply', async () => {
        await truffleAssert.reverts(crpPool.createPool(toWei('0')), 'ERR_INIT_SUPPLY_MIN');
    });

    it('crpPool should not createPool with BPT Initial Supply < MIN', async () => {
        await truffleAssert.reverts(crpPool.createPool(toWei('10')), 'ERR_INIT_SUPPLY_MIN');
    });

    it('crpPool should not createPool with BPT Initial Supply > MAX', async () => {
        await truffleAssert.reverts(crpPool.createPool(toWei('1000000001')), 'ERR_INIT_SUPPLY_MAX');
    });

    it('Non controller should not be able to createPool', async () => {
        await truffleAssert.reverts(crpPool.createPool(toWei('100'), { from: user1 }), 'ERR_NOT_CONTROLLER');
    });

    it('Non controller should not be able to createPool (with time params)', async () => {
        await truffleAssert.reverts(crpPool.createPool(toWei('100', 0, 0), { from: user1 }), 'ERR_NOT_CONTROLLER');
    });
    ...
    it('should not be able to createPool twice', async () => {
        await truffleAssert.reverts(crpPool.createPool(toWei('100')), 'ERR_IS_CREATED');
    });

    it('crpPool should not be able to set the cap without the right', async () => {
        await truffleAssert.reverts(crpPool.setCap(toWei('1000')), 'ERR_CANNOT_CHANGE_CAP');
    });
    ...
    it('Bpool should have matching protocol fee', async () => {
    ...
    it('Should not allow joining with partial token list', async () => {
        await truffleAssert.reverts(crpPool.joinPool(toWei('1'), [MAX, MAX]), 'ERR_AMOUNTS_MISMATCH');
    });

    it('Should not allow joining with zero out', async () => {
        await truffleAssert.reverts(crpPool.joinPool(toWei('0'), [MAX, MAX, MAX]), 'ERR_MATH_APPROX');
    });
    ...
    it('Fails calling any swap on unbound token', async () => {
        await truffleAssert.reverts(crpPool.joinswapExternAmountIn(XXX, toWei('2.5'), toWei('0')), 'ERR_NOT_BOUND');
        await truffleAssert.reverts(crpPool.joinswapPoolAmountOut(XXX, toWei('2.5'), MAX), 'ERR_NOT_BOUND');
        await truffleAssert.reverts(crpPool.exitswapPoolAmountIn(XXX, toWei('2.5'), toWei('0')), 'ERR_NOT_BOUND');
        await truffleAssert.reverts(crpPool.exitswapExternAmountOut(XXX, toWei('2.5'), MAX), 'ERR_NOT_BOUND');
    });

    // it('Failing calling any swap without whitelist permission', async () => {
    //     await truffleAssert.reverts(
    //         crpPool.joinswapExternAmountIn(WETH, toWei('1'), toWei('0'), {from: nonWhitelistUser}),
    //         "FCXAccessControl: sender requires permission"
    //     );
    //     await truffleAssert.reverts(
    //         crpPool.joinswapPoolAmountOut(WETH, toWei('2.5'), toWei('0')), {from: nonWhitelistUser},
    //         "FCXAccessControl: sender requires permission"
    //     );
    //     await truffleAssert.reverts(
    //         crpPool.exitswapPoolAmountIn(DAI, toWei('2.5'), toWei('0'), {from: nonWhitelistUser}),
    //         "FCXAccessControl: sender requires permission"
    //     );
    //     await truffleAssert.reverts(
    //         crpPool.exitswapExternAmountOut(DAI, toWei('2.5'), MAX, {from: nonWhitelistUser}),
    //         "FCXAccessControl: sender requires permission"
    //     );
    // });
    ...
    it('should not allow exiting with an amount mismatch', async () => {
        await truffleAssert.reverts(crpPool.exitPool(toWei('1'), [toWei('2.5')]), 'ERR_AMOUNTS_MISMATCH');
    });
    ...
    describe('PCToken interactions', () => {
        ...
        it('Token transfers', async () => {
            await truffleAssert.reverts(crpPool.transferFrom(user2, admin, toWei('10')), 'ERR_PCTOKEN_BAD_CALLER');
            ...
            await truffleAssert.reverts(crpPool.transfer(admin, toWei('10000'), { from: user2 }), 'ERR_SUB_UNDERFLOW');
        });
    });
});
