...
contract('pausableSwap', async (accounts) => {
    ...
    it('Set public swap should revert for non-controller', async () => {
        await truffleAssert.reverts(crpPool.setPublicSwap(false, { from: user }), 'ERR_NOT_CONTROLLER');
    });
    ...
    it('Non-controller should not be able to restart trades', async () => {
        await truffleAssert.reverts(crpPool.setPublicSwap(true, { from: user }), 'ERR_NOT_CONTROLLER');
    });
    ...
    it('Should not allow swaps while paused', async () => {
        ...
        await truffleAssert.reverts(
            bPool.swapExactAmountIn(DAI, toWei('500'), WETH, toWei('0'), MAX, { from: user }),
            'ERR_SWAP_NOT_PUBLIC'
        );

        await truffleAssert.reverts(
            bPool.swapExactAmountOut(DAI, MAX, WETH, toWei('1'), MAX, { from: user }),
            'ERR_SWAP_NOT_PUBLIC'
        );
    });
    ...
    it('Controller should not be able to change swapFee', async () => {
        await truffleAssert.reverts(crpPool.setSwapFee(toWei('1')), 'ERR_NOT_CONFIGURABLE_SWAP_FEE');
    });
