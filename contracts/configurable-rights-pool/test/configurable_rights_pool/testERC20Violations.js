...
contract('testERC20 violations', async (accounts) => {
    ...
    it('should not be able to add a non-conforming token (0 transfer)', async () => {
        await truffleAssert.reverts(crpPool.commitAddToken(LEND, toWei('10000'), toWei('1.5')), 'ERR_NO_ZERO_XFER');
    });

    it('should not be able to add a non-conforming token (returns false)', async () => {
        await truffleAssert.reverts(crpPool.commitAddToken(BRET, toWei('10'), toWei('1.5')), 'ERR_NONCONFORMING_TOKEN');
    });
    ...
    it('should not be able to create with a non-conforming token (no zero xfer)', async () => {
        ...
        await truffleAssert.reverts(crpFactory.newCrp(bFactory.address, poolParams, permissions), 'ERR_NO_ZERO_XFER');
    });
