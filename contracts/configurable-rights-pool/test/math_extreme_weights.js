...
contract('BPool', async (accounts) => {
    ...
    const { toWei, toTwosComplement } = web3.utils;
    ...
    describe('Extreme weights', () => {
        it('swapExactAmountIn', async () => {
            ...
            const output = await pool.swapExactAmountIn.call(tokenIn, tokenInAmount, tokenOut, minAmountOut, maxPrice);
        ...
        it('exitswapExternAmountOut should revert', async () => {
            ...
            // using twosComplement to pass an negative number
            await truffleAssert.reverts(
                pool.exitswapExternAmountOut(DAI, toTwosComplement(toWei(String(tokenAmountOut))), MAX),
                'ERR_MAX_OUT_RATIO'
            );
        });
