...
contract('BPool', async (accounts) => {
    ...
    describe('Binding Tokens', () => {
        ...
        it('Fails binding weights and balances outside MIX MAX', async () => {
            ...
            await truffleAssert.reverts(pool.bind(MKR, toWei('0.000000000000001'), toWei('1')), 'ERR_MIN_BALANCE');
    ...
    describe('User interactions', () => {
        ...
        */
        ...
        it('swapExactAmountIn', async () => {
            ...
            const txr = await pool.swapExactAmountIn(WETH, toWei('2.5'), DAI, toWei('475'), toWei('200'), {
                from: user2,
            });
        ...
        it('swapExactAmountOut', async () => {
            ...
            const txr = await pool.swapExactAmountOut(WETH, toWei('3'), MKR, toWei('1.0'), toWei('500'), {
                from: user2,
            });
            const log = txr.logs[0];
            assert.equal(log.event, 'LOG_SWAP');
        ...
        it('Fails joins exits with limits', async () => {
            ...
            await truffleAssert.reverts(pool.joinswapExternAmountIn(DAI, toWei('100'), toWei('10')), 'ERR_LIMIT_OUT');

            await truffleAssert.reverts(pool.joinswapPoolAmountOut(DAI, toWei('10'), toWei('100')), 'ERR_LIMIT_IN');

            await truffleAssert.reverts(pool.exitswapPoolAmountIn(DAI, toWei('1'), toWei('1000')), 'ERR_LIMIT_OUT');

            await truffleAssert.reverts(pool.exitswapExternAmountOut(DAI, toWei('1000'), toWei('1')), 'ERR_LIMIT_IN');
        });
        ...
        it('Fails calling any swap on unbound token', async () => {
            ...
            await truffleAssert.reverts(pool.joinswapExternAmountIn(XXX, toWei('2.5'), toWei('0')), 'ERR_NOT_BOUND');
            await truffleAssert.reverts(pool.joinswapPoolAmountOut(XXX, toWei('2.5'), MAX), 'ERR_NOT_BOUND');
            await truffleAssert.reverts(pool.exitswapPoolAmountIn(XXX, toWei('2.5'), toWei('0')), 'ERR_NOT_BOUND');
            await truffleAssert.reverts(pool.exitswapExternAmountOut(XXX, toWei('2.5'), MAX), 'ERR_NOT_BOUND');
        });
    ...
    describe('BToken interactions', () => {
        it('Token descriptors', async () => {
            ...
            assert.equal(name, 'FCX Pool Token');
        ...
        it('Token transfers', async () => {
            await truffleAssert.reverts(pool.transferFrom(user2, admin, toWei('10')), 'ERR_BTOKEN_BAD_CALLER');
    ...
    describe('Access control interactions', () => {
        it('Get access control address', async () => {
            const aclAddress = await pool.getAccessControlAddress();
            assert.equal(aclAddress, fcxAclInstance.address);
        });

        it('Only pool controller can set roles', async () => {
            await truffleAssert.reverts(
                pool.setRoles([adminRole, retrictedRole, unRetrictedRole], { from: user1 }),
                'ERR_NOT_CONTROLLER'
            );
        });

        it('Set roles', async () => {
            await pool.setRoles([adminRole, retrictedRole, unRetrictedRole]);
            const roles = await pool.getRoles();
            console.log(roles);
            console.log([adminRole, retrictedRole, unRetrictedRole]);
            assert.deepEqual(roles, [adminRole, retrictedRole, unRetrictedRole]);
        });

        it('Can set invalid access control address', async () => {
            const zeroAddress = '0x0000000000000000000000000000000000000000';
            await truffleAssert.reverts(
                pool.setAccessControlAddress(zeroAddress),
                'ERR_INVALID_ADDRESS'
            );
        });

        it('Only controller can set access control address', async () => {
            await truffleAssert.reverts(
                pool.setAccessControlAddress(fcxAclInstance.address, { from: user1 }),
                'ERR_NOT_CONTROLLER'
            );
        });
        ...
        it('Set access control address', async () => {
            ...
            await pool.setAccessControlAddress(newAcl.address);
            assert.equal(newAcl.address, await pool.getAccessControlAddress());
        });
    });

    describe('Reentry', () => {
        it('Cant reentry from a bad token', async () => {
            const newPoolAddress = await factory.newBPool.call();
            await factory.newBPool();
            newPool = await IBPool.at(newPoolAddress);
            const badToken = await ReEntryToken.new('ReEntry', 'ReEntry', 18, newPoolAddress);
            await badToken.mint(admin, toWei('50'));
            await badToken.approve(newPoolAddress, MAX);
            await truffleAssert.reverts(
                newPool.bind(badToken.address, toWei('50'), toWei('5')),
                'ERR_REENTRY'
            );
        });
    });
