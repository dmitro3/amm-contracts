    ...
    describe('Binding Tokens', () => {
        ...
        it('Fails swapExactAmountIn with limits', async () => {
            ...
            await truffleAssert.reverts(
                pool.swapExactAmountIn(AAA, toWei('1'), BBB, toWei('0'), toWei('0.9')),
                'ERR_BAD_LIMIT_PRICE'
            );
            await truffleAssert.reverts(
                pool.swapExactAmountIn(AAA, toWei('1'), BBB, toWei('2'), toWei('3.5')),
                'ERR_LIMIT_OUT'
            );
            await truffleAssert.reverts(
                pool.swapExactAmountIn(AAA, toWei('1'), BBB, toWei('0'), toWei('3.00001')),
                'ERR_LIMIT_PRICE'
            );
        });
        ...
        it('Fails swapExactAmountOut with limits', async () => {
            await truffleAssert.reverts(
                pool.swapExactAmountOut(AAA, toWei('51'), BBB, toWei('40'), toWei('5')),
                'ERR_MAX_OUT_RATIO'
            );
            await truffleAssert.reverts(
                pool.swapExactAmountOut(AAA, toWei('5'), BBB, toWei('1'), toWei('1')),
                'ERR_BAD_LIMIT_PRICE'
            );
            await truffleAssert.reverts(
                pool.swapExactAmountOut(AAA, toWei('1'), BBB, toWei('1'), toWei('5')),
                'ERR_LIMIT_IN'
            );
            await truffleAssert.reverts(
                pool.swapExactAmountOut(AAA, toWei('5'), BBB, toWei('1'), toWei('3.00001')),
                'ERR_LIMIT_PRICE'
            );
        });
    });
